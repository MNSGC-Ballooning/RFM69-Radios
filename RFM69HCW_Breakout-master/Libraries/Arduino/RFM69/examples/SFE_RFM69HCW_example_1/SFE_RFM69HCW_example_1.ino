...
//Mock temperature sensor stuff

#define HONEYWELL_TEMP A14 // Defining the analog pin
...
void loop()
{
  currentTempC = readTemp(); // converting to celcius
...
//  num++;
  ...
  {
    float input = currentTempC;
    dtostrf(input,5,2,sendbuffer);
    Serial.println(dtostrf(input,5,2,sendbuffer));
    Serial.println(input);
    if (input != '\r') // not a carriage return
    {
      sendbuffer[sendlength] = dtostrf(input,5,2,sendbuffer);
      sendlength++;
    }

    // If the input is a carriage return, or the buffer is full:
    
    if ((input == '\r') || (sendlength == 6)) // CR or buffer full
    {
      // Send the packet!
...
float readTemp(){ // function returns the temperature in Celcius
